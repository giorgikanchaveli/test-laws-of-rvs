...
function wass_distance(atoms_1::Vector{Float64}, atoms_2::Vector{Float64},
                       weights_1::Vector{Float64},weights_2::Vector{Float64}, p::Int)

    # this function is used already simulated empirical measures, i.e. we knot
    # atoms and weights of empirical probability measures
...
function wass_distance(pm_1::PM, pm_2::PM, n_rv::Vector{Int}, S::Int, p::Int)
    ...
    for i in 1:nrows
        for s in 1:S
            atoms_1, weights_1 = emp_distr(pm_1, n_rv[i])
            atoms_2, weights_2 = emp_distr(pm_2,n_rv[i])
            d[i, s] = wass_distance(atoms_1,atoms_2,weights_1,weights_2, p)
        end
    end
    return d
end
...
#     #compute disatnce
#     if dist == "wass"
#         return wass_distance(atoms_1,atoms_2,weights_1,weights_2, p)
#     elseif dist == "mmd"
